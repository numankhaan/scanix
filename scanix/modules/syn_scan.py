...
def syn_scan_port(target, port, timeout=1):
    result = {"target": target, "port": port, "protocol": "tcp_syn", "status": "filtered"}
    try:
        ...
        safe_print("SYN scan requires root/administrator privileges. Aborting SYN scan.", error=True)
        raise
